...
INSTALL=['install_once.sh','u2w_mainvideo_relay','u2wvideo-relay-start.cgi','u2wvideo-relay-status.cgi','u2whud_cast_relay_v8151']
UNINSTALL=['uninstall_once.sh','u2w_mainvideo_relay_v823','u2wvideo-relay-start-v823.cgi','u2wvideo-relay-status-v823.cgi','u2whud_cast_relay_v8151']
...
for names,once,outname in [
    (INSTALL,'install_once.sh','U2W_Update_v8.24_RecentIDRRelay.img'),
    (UNINSTALL,'uninstall_once.sh','U2W_Update_v8.24_RecentIDRRelay_UNINSTALL.img')]:
    ...
    digest=sha(w); rows.append((outname,digest)); print(outname,len(w),digest,members)
(BASE/'U2W_v8.24_SHA256SUMS.txt').write_text(''.join(f'{d}  {n}\n' for n,d in rows))
