/* U2W v8.20 validated-GOP Annex-B H.264 CGI streamer.
 *
 ...
 *
 * v8.20 keeps that transport behavior but hardens the GOP selector. Candidate
 * SPS/PPS/IDR NALs must pass header, size, profile, order, and proximity checks
 * before a new/reseeded HTTP client is bootstrapped from them.
 */
...
/* v8.20 bootstrap validation.
 *
 * The rolling exporter file contains genuine H.264 interleaved with unrelated
 * bytes. A byte-only "NAL type" scan can therefore mistake an accidental
 * 00 00 01 67/68/65 pattern in non-video data for the newest GOP. Validate the
 * candidate NAL lengths, reference bits, SPS profile byte, ordering and
 * proximity before a new HTTP client is seeded from it.
 */
static int plausible_sps_profile(unsigned char p){
    return p==66||p==77||p==88||p==100||p==110||p==122||p==244;
}
static void consider_completed_nal(
    int type,unsigned char header,unsigned char second,int have_second,
    off_t start,off_t header_off,off_t end,
    off_t*last_sps,off_t*last_pps,off_t*best_sps,off_t*best_pps,off_t*best_idr
){
    off_t n=end-header_off;
    if(start<0||header_off<0||n<=0)return;
    if((header&0x80)!=0)return; /* forbidden_zero_bit */
    if(type==7){
        if((header&0x60)==0||!have_second||n<4||n>128||!plausible_sps_profile(second))return;
        *last_sps=start;
        return;
    }
    if(type==8){
        if((header&0x60)==0||n<2||n>64)return;
        *last_pps=start;
        return;
    }
    if(type==5){
        if((header&0x60)==0||n<32||n>(off_t)(512*1024))return;
        if(*last_sps<0||*last_pps<0)return;
        if(!(*last_sps<*last_pps&&*last_pps<start))return;
        /* Real CarPlay parameter sets are compact and immediately precede the
         * decoder-safe GOP. These generous bounds reject unrelated false hits. */
        if((*last_pps-*last_sps)>4096)return;
        if((start-*last_pps)>65536)return;
        *best_sps=*last_sps;*best_pps=*last_pps;*best_idr=start;
    }
}
...
static int newest_gop(int fd,off_t*osps,off_t*opps,off_t*oidr){
    ...
    off_t cur_start=-1,cur_header_off=-1;
    int cur_type=-1,cur_have_second=0,expect_header=0,zeros=0;
    unsigned char cur_header=0,cur_second=0;
    ...
    for(;;){
        ...
        for(long i=0;i<r;i++,absolute++){
            ...
            if(expect_header){
                cur_header=b;cur_type=b&0x1f;cur_header_off=absolute;
                cur_have_second=0;expect_header=0;zeros=0;continue;
            }
            if(cur_header_off>=0&&!cur_have_second){cur_second=b;cur_have_second=1;}
            if(b==0){zeros++;continue;}
            if(b==1&&zeros>=2){
                int z=zeros>=3?3:2;off_t new_start=absolute-z;
                consider_completed_nal(cur_type,cur_header,cur_second,cur_have_second,
                    cur_start,cur_header_off,new_start,&last_sps,&last_pps,&best_sps,&best_pps,&best_idr);
                cur_start=new_start;cur_header_off=-1;cur_type=-1;cur_have_second=0;expect_header=1;
            }
            zeros=0;
        }
    }
    consider_completed_nal(cur_type,cur_header,cur_second,cur_have_second,
        cur_start,cur_header_off,absolute,&last_sps,&last_pps,&best_sps,&best_pps,&best_idr);
