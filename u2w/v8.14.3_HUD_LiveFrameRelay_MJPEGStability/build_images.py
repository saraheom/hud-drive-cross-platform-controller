...
INSTALL=['install_once.sh','u2whud_cast_relay','u2whud_discovery','u2whud_frame_ingress','u2whud_test.jpg','u2whud_stop.sh','u2whud-start.cgi','u2whud-stop.cgi','u2whud-status.cgi']
def sha(b): return hashlib.sha256(b).hexdigest()
def tar_payload(uninstall=False):
 names=['uninstall_once.sh'] if uninstall else INSTALL; bio=io.BytesIO()
 with gzip.GzipFile(filename='',mode='wb',fileobj=bio,mtime=0,compresslevel=9) as gz:
  with tarfile.open(fileobj=gz,mode='w',format=tarfile.GNU_FORMAT) as tf:
   for name in names:
    ...
    ti=tarfile.TarInfo(arc);ti.size=len(data);ti.uid=ti.gid=0;ti.uname=ti.gname='root';ti.mtime=0;ti.mode=0o755;tf.addfile(ti,io.BytesIO(data))
 return bio.getvalue()
def aes(data,dec=False):
 ...
 with tempfile.TemporaryDirectory() as td:
  ...
  subprocess.run(cmd,check=True,stdout=subprocess.DEVNULL,stderr=subprocess.DEVNULL); return op.read_bytes()+tail
for uninstall,outname in [(False,'U2W_Update_v8.14.3_HUD_LiveFrameRelay_MJPEGStability.img'),(True,'U2W_Update_v8.14.3_HUD_LiveFrameRelay_MJPEGStability_UNINSTALL.img')]:
 t=tar_payload(uninstall);w=aes(t);assert aes(w,True)==t;(BASE/outname).write_bytes(w)
