...
echo 'U2W HUD Live Frame Relay v8.14.3 status'
echo "marker=$([ -f /etc/u2whud_bridge_v8_14_3.marker ] && echo YES || echo NO)"
...
if grep -q 'hud-mjpeg-client-connected' /tmp/u2whud_cast.log 2>/dev/null; then echo 'hud_client_seen=YES'; else echo 'hud_client_seen=NO'; fi
if grep -q 'fallback-frame-sent' /tmp/u2whud_cast.log 2>/dev/null; then echo 'fallback_frame_sent=YES'; else echo 'fallback_frame_sent=NO'; fi
if grep -q 'live-frame-sent' /tmp/u2whud_cast.log 2>/dev/null; then echo 'live_frame_sent=YES'; else echo 'live_frame_sent=NO'; fi
