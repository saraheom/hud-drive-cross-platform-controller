/* U2W v8.23 live-IDR MainVideo H.264 relay.
 *
 ...
 *
 * Wire protocol:
 *   8 bytes ASCII "U2WH2642"
 *   repeated: [u32 big-endian NAL length][NAL bytes without Annex-B prefix]
 *
 * Critical v8.23 rule: there is NO cached GOP replay. A newly connected iPhone
 * waits for the next naturally arriving live IDR. At that boundary the relay
 * sends the latest validated SPS + PPS, then that fresh IDR, then forwards only
 * subsequently arriving live VCL/parameter NALs. This prevents the v8.22
 * 12-20 MiB historical bootstrap burst and keeps the client at the live edge.
 */
