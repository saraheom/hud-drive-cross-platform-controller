...
struct ConnectionCard: View {
    ...
    var selectedDevice: HudBluetoothManager.Device? {
    ...
    var body: some View {
            ...
            VStack(alignment: .leading, spacing: 12) {
                HStack {
                    VStack(alignment: .leading) {
                        Text("HUD DRIVE").font(.caption).foregroundStyle(.secondary)
        ...
        .onChange(of: state.bluetooth.devices) { _, devices in
            if selectedDeviceID == nil,
               let hud = devices.first(where: { $0.name.localizedCaseInsensitiveContains("HUD") }) {
