        ...
        VStack(alignment: .leading, spacing: 9) {
            ...
            DisclosureGroup(isExpanded: $showRelayDiagnostics) {
                VStack(alignment: .leading, spacing: 7) {
                    ...
                    Text("v90.35.3.24.9 pairs with U2W v8.28. v8.28 addresses the focus-dependent MainVideo bottleneck found on 9/23: the selector no longer assumes one numeric AppleCarPlay fd is MainVideo forever. It uses a non-blocking, bounded SPS→PPS→IDR proof before promoting a replacement source, performs no hot-path status/log writes, and never kills/signals/restarts AppleCarPlay. The standalone relay fences every mirror generation as a new codec epoch, and iOS treats VideoToolbox codecBadDataErr (-8969) as source-epoch corruption: TCP stays up while sanitizer/decoder state waits for fresh SPS/PPS/IDR. OBD ZIP collection is manual and no longer blocked by stale GPS speed. For the first validation, 10 fps remains the recommended validation cadence; leave the CarPlay screen unchanged and use LIVE • 5m continuity verified as the extended stability target.")
                        .font(.caption2)
                        .foregroundStyle(.secondary)
